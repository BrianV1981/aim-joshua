...
1. **Search:** Use `python3 aim-agy_os/.aim-agy_os/.aim_core/aim_cli.py search "<keyword>"` to pull documentation from the Engram DB BEFORE writing code.
...
- **In-Scope:** You have full autonomy to create, modify, and delete files (including writing required TDD tests) that are directly necessary to resolve the active `python3 aim-agy_os/.aim-agy_os/.aim_core/aim_cli.py fix <id>` ticket or assigned task.
- **Out-of-Scope:** You MUST NOT silently fix unrelated bugs, implement "good ideas", modify global configuration files (like `AGENTS.md`), or alter the testing environment unless explicitly commanded. If you encounter an out-of-scope issue, you MUST pause, ask the Operator, or open a new `python3 aim-agy_os/.aim-agy_os/.aim_core/aim_cli.py bug` ticket.
...
1. **Report:** Use `python3 aim-agy_os/.aim-agy_os/.aim_core/aim_cli.py bug "description"` (or enhancement) to log the issue. You MUST provide the `--context`, `--failure`, and `--intent` flags to bypass interactive prompts and ensure the next agent inherits full epistemic certainty.
...
If you need information about this project, the codebase, or your own rules, execute `python3 aim-agy_os/.aim-agy_os/.aim_core/aim_cli.py search` for the specific files below:
- **My Operating Rules:** `python3 aim-agy_os/.aim-agy_os/.aim_core/aim_cli.py search "A_I_M_HANDBOOK.md"` (This is an Index Card. Read it to find the specific `POLICY_*.md` file you need, then run a second search to read that specific policy).
...
- **The Operator Profile:** `python3 aim-agy_os/.aim-agy_os/.aim_core/aim_cli.py search "OPERATOR_PROFILE.md"`
...
1. **The Knowledge Map (`python3 aim-agy_os/.aim-agy_os/.aim_core/aim_cli.py map`):** Run this first to see a lightweight index of all loaded documentation titles. 
2. **Hybrid Search (`python3 aim-agy_os/.aim-agy_os/.aim_core/aim_cli.py search "<query>"`):** You must use the `run_shell_command` tool to execute this absolute command to search the Engram DB.
...
**Your immediate reflex must be to refer to the Engram DB via the `python3 aim-agy_os/.aim-agy_os/.aim_core/aim_cli.py search` command.**
- **The Context Window Fallacy:** Never rely solely on your conversational history, recent memory, or base training weights to answer factual questions. You MUST execute a fresh `python3 aim-agy_os/.aim-agy_os/.aim_core/aim_cli.py search` against the internal databases before formulating an answer.
- If you hit an error, execute `python3 aim-agy_os/.aim-agy_os/.aim_core/aim_cli.py search "<Error String or Function Name>"` to look there FIRST.
...
1. **The Architecture:** Read `python3 aim-agy_os/.aim-agy_os/.aim_core/aim_cli.py search "Reincarnation-Map.md"` to understand how your "Will" is passed to the next vessel.
...
3. **Worktree Hygiene:** A.I.M. creates isolated Git Worktrees in the `workspace/` directory for each issue (`python3 aim-agy_os/.aim-agy_os/.aim_core/aim_cli.py fix <id>`). To prevent the CLI from recursively scanning hundreds of redundant files across multiple branches, you MUST ensure that `workspace/` is listed in your `.opencodeignore` file. When an issue is complete, actively clean up the worktree using `python3 aim-agy_os/.aim-agy_os/.aim_core/aim_cli.py promote` or `git worktree remove` to prevent context bloat.
...
1. Run `python3 aim-agy_os/.aim-agy_os/.aim_core/aim_cli.py pulse` to manually generate a handoff document.
2. **Agentic Reincarnation Protocol:** When the Operator types `/reincarnate` or `/python3 aim-agy_os/.aim-agy_os/.aim_core/aim_cli.py reincarnate`, you MUST manually execute the handoff. Use the `run_shell_command` tool to:
...
- **To Write:** DO NOT manually edit the wiki pages. If you learn a new "Eureka" moment or have a new document to add, write the raw text file into `memory-wiki/_ingest/` and execute `python3 aim-agy_os/.aim-agy_os/.aim_core/aim_cli.py wiki process` to hand it off to the Subconscious Daemon.

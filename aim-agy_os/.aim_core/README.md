...
This directory contains the internal Python engine that powers the A.I.M. exoskeleton. These files are typically NOT invoked directly by the Operator; they are called by the `aim_core/aim_cli.py` router, the background `daemon`, or the Gemini CLI `hooks/`.

...
2. **Blackbox Vault:** Before exiting the current session, the agent MUST seal the session into the immutable blackbox vault by executing a vessel-specific blackbox command (e.g., `aim agy-blackbox --session-id <uuid>`, `aim grok-blackbox`, etc.). This extracts raw session history (SQLite/JSONL) natively.

...
You have been injected with highly specialized knowledge cartridges. Before proposing a plan or writing code, you MUST use 'aim-joshua search \"<concept>\"' to pull the definitive architectural rules from your Engram DB. Never hallucinate APIs.
...
1. **Report:** Use 'aim-joshua bug \"desc\"' to log the issue.
2. **Isolate:** You MUST use 'aim-joshua fix <id>' to check out an isolated branch.
3. **Release:** Use 'aim-joshua push \"msg\"' only when the TDD pipeline is green.
"
...
# 2. Directory Naming: The A.I.M. clone MUST be named 'joshua_os' (or similar) to prevent the agent from getting confused 
#    between the A.I.M. framework code and the target test code.
# 3. The Workstation: The actual test codebase (e.g. django_repo) and the TASK.md MUST be placed inside an 'joshua_os/workspace' folder. 
...
setup_arena() {
    ...
    if [ "$type" == "control" ]; then
        ...
        # Matrix arenas require the strict joshua_os -> workspace structural mandate
        git clone --depth 1 "$AIM_SOURCE" joshua_os >/dev/null 2>&1
        cd joshua_os
        ...
        # Build the isolated workspace *inside* joshua_os
        mkdir -p workspace
        cd workspace
        echo "$TASK_CONTENT" > TASK.md
...
echo "To test a Matrix Agent, remember to 'cd joshua_os', 'source ~/.bashrc', and use 'aim-joshua status'!"
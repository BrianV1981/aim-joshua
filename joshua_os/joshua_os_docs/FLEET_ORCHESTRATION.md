...
| **Soul** | `aim-joshua` | `/home/kingb/aim-joshua` | Host-agnostic engine fixes, GitHub issues on BrianV1981/aim-joshua, merge to soul main |
...
Each vessel records the aim-joshua commit it claims to track:

| Field | Meaning |
|-------|---------|
| Commit / Short | aim-joshua `main` SHA after intentional sync |
...
Host-agnostic code lives on **aim-joshua** first. Vessel-specific files must not be clobbered:
...
tmux capture-pane -t aim-joshua -p -J -S -20   # idle?
```

2. Write a **chalkboard** under soul planning artifacts, e.g.:

`/home/kingb/aim-joshua/joshua_os/planning-artifacts/DISPATCH_FROM_GROK_<topic>.md`
...
bash /home/kingb/aim-grok/.grok/skills/aim-communicate/scripts/tmux_send.sh \
  ...
  --from "$FROM" \
  --reply-to "$FROM" \
  --message "DISPATCH: <one line>. Full orders: /home/kingb/aim-joshua/joshua_os/planning-artifacts/DISPATCH_….md"
```
...
4. Verify delivery: `tmux capture-pane -t aim-joshua -p -J -S -30` shows a **submitted** turn (`Working…` / tool use), not stuck composer text.
...
```text
[FROM:aim-joshua] [REPLY_TO:<orchestrator>] [REPORT from aim-joshua] MERGED #N — main=<sha> — read <REPORT.md>
```
...
Full `rsync --delete` of `joshua_os/` often shows drift on `aim_cli.py` and pulse modules because **Grok already carries intentional overlays**. Blind rsync loses them.

**Surgical path (used for #53):**

1. Extract the soul hunks (or re-apply the same `input()` gates by hand).
2. Patch grok `joshua_os/.aim_core/<module>.py`.
3. Patch opencode nested `joshua_os/.aim_core/<module>.py` (canonical layout).
...
| Session | CLI | Cwd |
|---------|-----|-----|
| `aim-joshua` | `agy` | `/home/kingb/aim-joshua` |
...
Is the change host-agnostic engine / CLI / memory / GitOps?
  YES → aim-joshua implements on worktree; orchestrator audits + pin-syncs
...
Path: `aim-joshua/joshua_os/planning-artifacts/DISPATCH_FROM_GROK_<TOPIC>.md`
...
| Dispatch | → `aim-joshua` | Close 57+5; implement 53; no merge until audit |
...
| Orchestrator implements on soul | Medium-high | Soul pane dead / Operator scopes grok to edit aim-joshua |
...
- “Talk to aim-joshua …” → permission to message that session
...
6. `gh issue list --repo BrianV1981/aim-joshua --state open` (or local tracker).
...
- Do not modify `/home/kingb/aim-joshua` from grok unless Operator scopes it (prefer dispatching soul).
...
FROM=$(tmux display-message -p '#{session_name}')
bash /home/kingb/aim-grok/.grok/skills/aim-communicate/scripts/tmux_send.sh \
  --target aim-joshua --from "$FROM" --reply-to "$FROM" \
  --message "DISPATCH: … — read /abs/path.md"

# Soul health / issues
cd /home/kingb/aim-joshua && gh issue list --state open
cd /home/kingb/aim-joshua && git rev-parse HEAD && git log -1 --oneline
...
cd /home/kingb/aim-opencode && python3 joshua_os/.aim_core/aim_cli.py doctor
```

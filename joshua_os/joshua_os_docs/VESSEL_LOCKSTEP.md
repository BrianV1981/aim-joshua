...
| Layer | Owner | Rule |
|-------|--------|------|
| Shared engine | **aim-joshua** | Implement host-agnostic fixes here first |
...
**Layout goal (Operator 2026-07-12):** all vessels use **nested** `joshua_os/` (same shape as agy/grok). Identical soul tree as far as practical; **harness overlays only** where the CLI differs (spawn binary, transcript paths, skills dir). aim-opencode migrates off flat `aim_core/`.
...
Canonical module should live on **aim-joshua** once (#95); vessels re-export or sync the same file.
